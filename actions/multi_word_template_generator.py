...
from utils.database_insert import insert_data
...
def generate_fallecimiento_template(doc, data_dict, currency, producto):
    ...
    operacion_table = doc.add_table(rows=1, cols=8)
    ...
    for data_row in data_dict:
        row_cells = operacion_table.add_row().cells
...
def generate_falta_ds_template(doc, data_dict, currency, producto):
    from actions.file_actions import translate_month_to_spanish, get_current_month, format_date
    # Add texto exclusion
...
def generate_sin_capital_template(doc, data_dict, currency, producto):
    from actions.file_actions import translate_month_to_spanish, get_current_month, format_date
    # Add texto exclusion
    ...
    # Add a table operacion
    operacion_table = doc.add_table(rows=1, cols=7)
    operacion_table.style = 'Table Grid'
    hdr_cells = operacion_table.rows[0].cells
    hdr_cells[0].text = 'Nombre del cliente'
    hdr_cells[1].text = 'Nro. Documento'
...
def generate_mora_template(doc, data_dict, currency, producto):
    from actions.file_actions import translate_month_to_spanish, get_current_month, format_date
    # Add texto exclusion
...
def generate_edad_superior_template(doc, data_dict, currency, producto):
    from actions.file_actions import translate_month_to_spanish, get_current_month, format_date
    # Add texto exclusion
...
def generate_edad_inferior_template(doc, data_dict, currency, producto):
    from actions.file_actions import translate_month_to_spanish, get_current_month, format_date
    # Add texto exclusion
...
def generate_operacion_vencida_template(doc, data_dict, currency, producto):
    from actions.file_actions import translate_month_to_spanish, get_current_month, format_date
    # Add texto exclusion
...
def generate_politica_suscripcion_template(doc, data_dict, currency, producto):
    from actions.file_actions import translate_month_to_spanish, get_current_month, format_date
    # Add texto exclusion
...
def generate_anulado_template(doc, data_dict, currency, producto):
    from actions.file_actions import translate_month_to_spanish, get_current_month, format_date
    # Add texto exclusion
...
def generate_diferencia_ds_template(doc, data_dict, currency, producto):
    from actions.file_actions import translate_month_to_spanish, get_current_month, format_date
    # Add texto exclusion
...
def generate_informacion_adicional_template(doc, data_dict, currency, producto):
    from actions.file_actions import translate_month_to_spanish, get_current_month, format_date
    # Add texto exclusion
    exclusion_paragraph = doc.add_paragraph("       Por la presente se informa la exclusión del Prestatario indicado a continuación, de la póliza de Seguro de Vida Colectivo para Cancelación de Deudas, por motivo de solicitud de información adicional con respecto a lo declarado en los ítems de la Declaración de Salud.")
    exclusion_paragraph.runs[0].font.name = 'Arial'
    exclusion_paragraph = doc.add_paragraph(("       La operación corresponde a la planilla de {producto} en moneda {moneda} del mes de {mes}.").format(producto=producto, moneda=currency, mes=translate_month_to_spanish(get_current_month())))
    exclusion_paragraph.alignment = WD_PARAGRAPH_ALIGNMENT.JUSTIFY
    exclusion_paragraph.runs[0].font.name = 'Arial'
...
def generate_exclusiones_previas_template(doc, data_dict, currency, producto):
    from actions.file_actions import translate_month_to_spanish, get_current_month, format_date
    # Add texto exclusion
...
def generate_operacion_adelantada_template(doc, data_dict, currency, producto):
    from actions.file_actions import translate_month_to_spanish, get_current_month, format_date
    # Add texto exclusion
...
def generate_persona_juridica_template(doc, data_dict, currency, producto):
    from actions.file_actions import translate_month_to_spanish, get_current_month, format_date
    # Add texto exclusion
...
def generate_cambio_condiciones_template(doc, data_dict, currency, producto):
    from actions.file_actions import translate_month_to_spanish, get_current_month, format_date
    # Add texto exclusion
...
def generate_analisis_template(doc, data_dict, currency, producto):
    from actions.file_actions import translate_month_to_spanish, get_current_month, format_date
    # Add texto exclusion
...
def generate_historial_cobertura_template(doc, data_dict, currency, producto):
    from actions.file_actions import translate_month_to_spanish, get_current_month, format_date
    # Add texto exclusion
...
def generate_no_renovado_template(doc, data_dict, currency, producto):
    from actions.file_actions import translate_month_to_spanish, get_current_month, format_date
    # Add texto exclusion
...
# Testing fuction 
def generate_testing_template(doc, data_rows, currency, producto):
    from actions.file_actions import translate_month_to_spanish, get_current_month, format_date
    # Add texto exclusion
    exclusion_paragraph = doc.add_paragraph("       Por la presente se informa la exclusión del Prestatario indicado a continuación, de la póliza de Seguro de Vida Colectivo para Cancelación de Deudas, debido a que su operación no cuenta con permanencia debido a que no fue remitida para renovar anteriormente.")
    exclusion_paragraph.runs[0].font.name = 'Arial'
    ...
    # Format table cell fonts and styles
    for row in operacion_table.rows:
        for cell in row.cells:
            font = cell.paragraphs[0].runs[0].font
            font.size = Pt(9)
...
def multi_generate_template_with_content(doc, entity_name, currency, producto, key, dict_items):
    ...
    template_name = str(key)    # Lugar de la lista donde se encuentra el motivo de la exclusion
    # Select the appropriate template acording to the exclusion reason
    if template_name == "FF1":
        generate_fallecimiento_template(doc, dict_items, currency, producto)
    elif template_name == "DS4":
        generate_falta_ds_template(doc, dict_items, currency, producto)
    elif template_name == "DS1":
        generate_ds_incompleta_template(doc, dict_items, currency, producto)
    elif template_name == "sin capital":
        generate_sin_capital_template(doc, dict_items, currency, producto)
    elif template_name == "MM1":
        generate_mora_template(doc, dict_items, currency, producto)
    elif template_name == "ED1":
        generate_edad_superior_template(doc, dict_items, currency, producto)
    elif template_name == "ED2":
        generate_edad_inferior_template(doc, dict_items, currency, producto)
    elif template_name == "OV1":
        generate_operacion_vencida_template(doc, dict_items, currency, producto)
    elif template_name == "PS1":
        generate_politica_suscripcion_template(doc, dict_items, currency, producto)
    elif template_name == "anulado":
        generate_anulado_template(doc, dict_items, currency, producto)
    elif template_name == "DS2":
        generate_diferencia_ds_template(doc, dict_items, currency, producto)    
    elif template_name == "DS3":
        generate_informacion_adicional_template(doc, dict_items, currency, producto)
    elif template_name == "EP1":
        generate_exclusiones_previas_template(doc, dict_items, currency, producto)
    elif template_name == "OA1":
        generate_operacion_adelantada_template(doc, dict_items, currency, producto)
    elif template_name == "PJ1":
        generate_persona_juridica_template(doc, dict_items, currency, producto)
    elif template_name == "CC1":
        generate_cambio_condiciones_template(doc, dict_items, currency, producto)
    elif template_name == "PP1":
        generate_analisis_template(doc, dict_items, currency, producto)
    elif template_name == "SH1":
        generate_historial_cobertura_template(doc, dict_items, currency, producto)
    elif template_name == "NR1":
        generate_no_renovado_template(doc, dict_items, currency, producto)
